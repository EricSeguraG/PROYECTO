...
const WishlistScreen = ({ onBack, user: propUser }) => {
  ...
  const loadWishlist = async (userId) => {
    ...
    setError(null);
    try {
      const data = await perfumeAPI.getWishlist(userId);
      setWishlist(data || []);
      ...
      loadPerfumeImages(data || []);
      
    } catch (err) {
      console.error('Error cargando wishlist:', err);
      setError('No se pudo cargar tu wishlist. Intenta de nuevo.');
    } finally {
      setLoading(false);
    }
  };
  ...
  // Obtener perfumes para la página actual
  const getCurrentPagePerfumes = () => {
...
const handleRemoveFromWishlist = async (perfumeId, perfumeName) => {
  if (!user) return;
  
  if (!window.confirm(`¿Eliminar "${perfumeName}" de tu wishlist?`)) {
    return;
  }
  ...
  try {
    ...
    // FILTRO CORREGIDO - Eliminar el item correcto
    const updatedWishlist = wishlist.filter(item => {
      // Comparar tanto por ID como por nombre
    ...
    const total = Math.ceil(updatedWishlist.length / itemsPerPage);
    setTotalPages(total || 1);
    ...
    setError('Error al eliminar de la wishlist. Intenta de nuevo.');
  }
};
      ...
      <div style={containerStyle}>
        ...
        <div style={{ 
          ...
          <p style={{ color: 'rgba(243, 229, 171, 0.6)', fontSize: '1rem' }}>
            Preparando tu colección de deseos
          </p>
        </div>
        <style jsx>{`
          @keyframes spin {
  ...
  const currentPagePerfumes = getCurrentPagePerfumes();
  const startIndex = (currentPage - 1) * itemsPerPage + 1;
  const endIndex = Math.min(currentPage * itemsPerPage, wishlist.length);
          ...
          <div className="card" style={{ maxWidth: '1400px', margin: '0 auto', width: '95%' }}>
            ...
            {!loading && (
              <div 
                ...
                ref={gridRef}
              >
                {currentPagePerfumes.length > 0 ? (
                  currentPagePerfumes.map((item, index) => {
                    // Obtener imagen del estado de imágenes
                    const imageUrl = perfumeImages[item.perfume_id || item.perfume_name];
                    
                    return (
                      <div 
                        key={`${item.perfume_id || index}-${currentPage}`}
                        style={{
                          background: 'rgba(0, 0, 0, 0.9)',
                          borderRadius: '8px',
                          border: '2px solid #F3E5AB',
                      ...
                      >
                        ...
                        <div style={{ 
                          ...
                          {/* Imagen cuadrada - AHORA CON IMAGEN REAL */}
                          <div style={{
                            width: '60px',
                            height: '60px',
                            flexShrink: 0,
                          ...
                          <div style={{ 
                            ...
                            <div>
                              ...
                              {/* FECHA AÑADIDA AQUÍ - igual que en CollectionScreen */}
                        ...
                        <div style={{ 
                          ...
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              handleRemoveFromWishlist(item.perfume_id, item.perfume_name);
                            }}
                            style={{
                  ...
                  <div style={{ 
                    ...
                    <div style={{ 
                      ...
                      <Heart size={36} color="rgba(243, 229, 171, 0.5)" />
                    </div>
                    <h3 style={{ 
                      color: '#F3E5AB', 
                      marginBottom: '1rem',
                    ...
                    <p style={{ 
                      ...
                      Añade perfumes a tu wishlist usando el corazón ❤️ en los detalles de cada perfume.
                    </p>
                    <button
                      onClick={onBack}
                      style={{
            ...
            {/* INFORMACIÓN FIJA - SIEMPRE VISIBLE */}
            <div style={{ 
              textAlign: 'center', 
              marginTop: '1.5rem',
              padding: '1rem',
            ...
            {!loading && totalPages > 1 && wishlist.length > 0 && (
              <div style={{
                display: 'flex',
                justifyContent: 'center',
                alignItems: 'center',
                ...
                <span style={{ color: '#F3E5AB', fontSize: '0.8rem', marginRight: '0.8rem' }}>
            ...
            {!loading && wishlist.length > 0 && (
              <div style={{ 
                ...
                <div style={{ display: 'flex', justifyContent: 'center', gap: '0.8rem', flexWrap: 'wrap' }}>
                  <span>
                    Mostrando {startIndex}-{endIndex} de {wishlist.length} perfumes
                  </span>
                </div>
              </div>
            )}
